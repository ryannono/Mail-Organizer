 ...
 * });
 */
export default function batchInputs(
  maxInputArraySize: number,
  rawInputArray: string[]
) {
  return Array.from(
    { length: Math.ceil(rawInputArray.length / maxInputArraySize) },
    (chunk, index) =>
      rawInputArray.slice(
        index * maxInputArraySize,
        index * maxInputArraySize + maxInputArraySize
      )
  );
}
